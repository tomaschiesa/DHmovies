...
http.createServer(function(req, res){
	...
	switch (req.url) {
	//Al recibir la uri '/' mostramos todo lo siguiente
	case '/':
		...
		res.write('            |--------------------------------------|'+'\n'+'\n');
		res.write('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>'+'\n');
		res.write('>>>  El mejor sitio para encontrar las mejores peliculas,  <<<'+'\n');
		res.write('>>>   incluso mucho mejor que Netflix, Cuevana y PopCorn.  <<<'+'\n');
		res.write('<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<'+'\n');

		//B)_ Mostrar el total de peliculas.
		let cantidadDePeliculasA = movies.length;
		res.write('\n'+'--------------------------------------------------------------'+'\n'+'\n');  //Division lineal entre consignas (Chequear write o end)
		res.write('         El total de peliculas en cartelera es de: ' + cantidadDePeliculasA+'\n');
		//C)_ Mostrar el listado de peliculas ordenado alfabeticamente.
		res.write('\n'+'--------------------------------------------------------------'+'\n'+'\n');  //Division lineal entre consignas (Chequear write o end)
		res.write('                  ** LISTADO DE PELICULAS **'+'\n'+'\n');
		let pelisOrdenadasA = [];
		for (let i=0; i<movies.length; i++){
			pelisOrdenadasA.push(movies[i].title);
		}
		pelisOrdenadasA = pelisOrdenadasA.sort();
		for (let i=0; i<pelisOrdenadasA.length; i++){
		res.write('   - ' + pelisOrdenadasA[i] +'\n')
		}
	...
	case '/en-cartelera':
		...
		res.write('           |--------------------------------------|'+'\n');
		//B)_ Mostrar el total de peliculas
		let cantidadDePeliculasB = movies.length;
		res.write('\n'+'         El total de peliculas en cartelera es de: ' + cantidadDePeliculasB+'\n');
		//C)_ Componer el listado de peliculas con titulo y reseña.
		res.write('\n'+'--------------------------------------------------------------'+'\n'+'\n');  //Division lineal entre consignas (Chequear write o end)
		res.write('\n'+'                  ** LISTADO DE PELICULAS **'+'\n'+'\n');
		let pelisOrdenadasB = [];
		for (let i=0; i<movies.length; i++){
			pelisOrdenadasB.push(movies[i].title+':' +'\n'+ '     Resumen de la pelicula: '+movies[i].overview+'\n');
		}
		pelisOrdenadasB = pelisOrdenadasB.sort();
		for (let i=0; i<pelisOrdenadasB.length; i++){
		res.write(' *** ' + pelisOrdenadasB[i] +'\n')
		}
		// Cierre decorativo de lineas
		res.end('\n'+'--------------------------------------------------------------'+'\n'+'\n');  //Division lineal entre consignas (Chequear write o end)
        break;
    case '/mas-votadas':
		//A)_ Dar una titulo a la seccion.
		res.write('           |--------------------------------------|'+'\n');
		res.write('           |        PELICULAS MAS VOTADAS!        |'+'\n');
		res.write('           |--------------------------------------|'+'\n');
		//B)_ Mostrar el total de peliculas que superen los 7 puntos de votacion.
		//contar las peliculas que movies.vote_average >=7
		let votosPelis = [];
		let cantidadDePeliculas7oMas = 0;
		for (let i=0; i<movies.length; i++){
			votosPelis.push(movies[i].vote_average);
		}
		for (let i=0; i<votosPelis.length; i++){
			if (votosPelis[i] >= 7){
			cantidadDePeliculas7oMas++;
		}
	}res.write('\n'+'         Las peliculas mas votadas en cartelera son: ' + cantidadDePeliculas7oMas +'\n');
		//C)_ Mostrar el rating promedio.
		res.write('\n'+'--------------------------------------------------------------'+'\n');  //Division lineal entre consignas (Chequear write o end)
		res.write('\n'+'           ** RATING PROMEDIO DE LAS MAS VOTADAS **'+'\n');
		let peliculas7oMas = [];
		let promedio = 0;
		for (let i=0; i<votosPelis.length; i++){
			if (votosPelis[i] >= 7){
				peliculas7oMas.push(votosPelis[i]);
		}
	}
		let reduceVotos = peliculas7oMas.reduce(function(acumulador, votosPelis){
			return acumulador + votosPelis;
		});
		promedio = reduceVotos / cantidadDePeliculas7oMas
		let promedioFixed = promedio.toFixed(2)
		res.write('              El promedio de los votos es de: ' + promedioFixed +'\n'); //me muestra un error con los decimales!!!!!!!!!!
		//D)_ Mostrar el titulo + rating + reseña.
		res.write('\n'+'--------------------------------------------------------------'+'\n');  //Division lineal entre consignas (Chequear write o end)
		res.write('\n'+'          ** LISTADO DE LAS PELICULAS MAS VOTADAS **');
		res.write('\n'+'                    (Por orden alfabetico)'+'\n'+'\n');
		let pelisOrdenadasC = [];
		for (let i=0; i<movies.length; i++){
			if (votosPelis[i] >= 7){
			pelisOrdenadasC.push(movies[i].title+':'+'\n'+ '     Rating: ' +movies[i].vote_average +'\n'+ '     Resumen de la pelicula: '+movies[i].overview+'\n');
		}
	}
		pelisOrdenadasC = pelisOrdenadasC.sort();
		for (let i=0; i<pelisOrdenadasC.length; i++){
		res.write(' *** ' + pelisOrdenadasC[i] +'\n')
		}
